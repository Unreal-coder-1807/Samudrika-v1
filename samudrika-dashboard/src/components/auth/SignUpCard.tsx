...
const parseClerkError = (err: unknown) => {
  if (typeof err === "object" && err !== null && "errors" in err) {
    ...
    return errors?.[0]?.message ?? "Sign up failed. Please try again.";
  }
...
export default function SignUpCard() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        setError("Please complete verification from your email before signing in.");
      }
    } catch (err) {
  ...
  return (
    <div style={{ width: "100%", maxWidth: "360px", display: "flex", flexDirection: "column", alignItems: "center", zIndex: 1, position: "relative" }}>
      <h1 style={{ fontFamily: "Inter, system-ui", fontSize: "40px", fontWeight: 700, color: "#FFFFFF", letterSpacing: "-0.025em", margin: "0 0 28px", textAlign: "center" }}>
        Create Account.
      </h1>
      ...
      <input className="auth-input" type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} style={{ marginBottom: "10px" }} />
      <input
      ...
      {error && <p style={{ fontSize: "12px", color: "#FF6B6B", fontFamily: "IBM Plex Mono, monospace", marginBottom: "12px", textAlign: "center" }}>{error}</p>}
      ...
      <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.45)" }}>
        ...
        <Link to="/sign-in" style={{ color: "#FFFFFF", fontWeight: 600, textDecoration: "none" }}>
          Sign in
        </Link>
      </p>
    </div>
